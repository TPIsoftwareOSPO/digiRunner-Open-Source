...
public class GtwIdPVgroupController {
	...
	//因為弱掃掃JDBC不能用GET,但又有其他產品共用,所以增加POST
	//Because weak scanning of JDBC cannot use GET, but other products share the same functionality, POST is added.
	@PostMapping(value = "/dgrv4/ssotoken/gtwidp/{idPType}/getVgroupList")
	public ResponseEntity<?> getVgroupPostListByPost(@RequestHeader HttpHeaders headers, 
			HttpServletRequest httpReq, 
			HttpServletResponse httpRes,
			@PathVariable("idPType") String idPType) throws Exception{
		GtwIdPVgroupResp resp = service.getVgroupList(headers, httpReq, httpRes, idPType);
