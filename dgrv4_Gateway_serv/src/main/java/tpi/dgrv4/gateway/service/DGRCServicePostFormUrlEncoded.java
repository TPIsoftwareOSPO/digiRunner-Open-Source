...
public class DGRCServicePostFormUrlEncoded implements IApiCacheService {
	...
	public void forwardToPostFormUrlEncoded(HttpHeaders httpHeaders, HttpServletRequest httpReq,
		...
		try {
			...
			// 第一組 RDB Req
			TsmpApiLogReq dgrcUrlEncodedDgrReqVo_rdb = getCommForwardProcService().addRdbTsmpApiLogReq1(uuid, httpReq, maskReqMbody, "dgrc", aType);
			...
			if (isMockTest) {
				...
				var mockResp = mockForwardTo(httpReq, httpRes, httpHeaders, srcUrl, uuid, tokenPayload, dgrcUrlEncodedDgrReqVo,
		...
		if ("1".equals(failHandlePolicy)) {// 1: 當調用目標URL失敗時,自動重試下一個目標URL
			...
			for (int i = 0; i < srcUrlList.size(); i++) {
				...
				convertResponseBodyMap = forwardTo(httpReq, httpRes, httpHeaders, srcUrl, values, uuid, tokenPayload,
						dgrcUrlEncodedDgrReqVo, dgrcUrlEncodedDgrReqVo_rdb, cApiKeySwitch, tryNumWord, apiReg, maskInfo);
			...
			TPILogger.tl.debug("Src Url:" + srcUrl);
			convertResponseBodyMap = forwardTo(httpReq, httpRes, httpHeaders, srcUrl, values, uuid, tokenPayload,
					dgrcUrlEncodedDgrReqVo, dgrcUrlEncodedDgrReqVo_rdb, cApiKeySwitch, null, apiReg, maskInfo);
		}
	...
	protected Map<String, Object> forwardTo(HttpServletRequest httpReq, HttpServletResponse httpRes,
											...
											Boolean cApiKeySwitch, String tryNumWord, TsmpApiReg apiReg, Map<String, String> maskInfo) throws Exception {
		...
		if (StringUtils.hasText(autoCacheId)) {// 自適應cache
			...
			if(apiCacheRespVo != null) {//走cache
				...
				dgrcUrlEncoded_respObj = this.callForwardApi(header, httpReq, httpRes, srcUrl, dgrcUrlEncodedDgrReqVo,
						reqMbody, uuid, values, false, tryNumWord, apiReg, maskInfo);
			}
			...
			if (cacheVo != null) {// 走cache
				...
				if (isUpdate) {// 更新紀錄
					dgrcUrlEncoded_respObj = this.callForwardApi(header, httpReq, httpRes, srcUrl,
							dgrcUrlEncodedDgrReqVo, reqMbody, uuid, values, true, tryNumWord, apiReg, maskInfo);
				...
				dgrcUrlEncoded_respObj = this.callForwardApi(header, httpReq, httpRes, srcUrl, dgrcUrlEncodedDgrReqVo,
						reqMbody, uuid, values, true, tryNumWord, apiReg, maskInfo);
				// statusCode大於等於200 且 小於400才更新紀錄
				if (dgrcUrlEncoded_respObj.statusCode >= 200 && dgrcUrlEncoded_respObj.statusCode < 400) {
	...
	public HttpRespData callback(AutoCacheParamVo vo, Map<String, String> maskInfo) {
		try {
			...
			TsmpApiLogReq dgrcUrlEncodedBackendReqVo = getCommForwardProcService().addEsTsmpApiLogReq2(vo.getDgrReqVo(), vo.getHeader(), vo.getSrcUrl(), maskReqMbody);

			HttpRespData respObj = getHttpRespData(vo.getHttpMethod(), vo.getHeader(), vo.getSrcUrl(), vo.getValues() , vo.getHttpReq(), maskInfo);
	...
	private HttpRespData callForwardApi(Map<String, List<String>> header, HttpServletRequest httpReq,
										...
										MultiValueMap<String, String> values, boolean isFixedCache, String tryNumWord
										, TsmpApiReg tsmpApiReg, Map<String, String> maskInfo) throws Exception {
		...
		TsmpApiLogReq backendReqVo = getCommForwardProcService().addEsTsmpApiLogReq2(dgrReqVo, header, srcUrl, maskReqMbody);
		HttpRespData respObj = getHttpRespData(httpReq.getMethod(), header, srcUrl, values , httpReq, maskInfo);
		respObj.fetchByte(maskInfo); // because Enable inputStream
	...
	private StringBuffer getLogReq(HttpServletRequest httpReq, HttpHeaders httpHeaders,
								   String reqUrl, MultiValueMap<String, String> values, Map<String, String> maskInfo) throws IOException {
		StringBuffer dgrcUrlEncoded_log = new StringBuffer();

		// print
		writeLogger(dgrcUrlEncoded_log, "--【URL】--");
		writeLogger(dgrcUrlEncoded_log, getCommForwardProcService().getUrlAddQueryString(httpReq, httpReq.getRequestURI()));
	...
	protected HttpRespData getHttpRespData(String httpMethod, Map<String, List<String>> header,
										   String srcUrl, MultiValueMap<String, String> values , HttpServletRequest httpReq, Map<String, String> maskInfo) throws Exception {
