...
public class DGRCServicePostRaw implements IApiCacheService {
	...
	public void forwardToPostRawData(HttpHeaders httpHeaders, HttpServletRequest httpReq, HttpServletResponse httpRes,
		...
		try {
			...
			if (opt_tsmpApiReg.isPresent()) {
				...
				throw new Exception(
						"TSMP_API_REG not found, api_key:" + apiId + "\t,module_name:" + dgrcPostRaw_moduleName);
			}
			...
			// 印出第一道log
			StringBuffer reqLog = getLogReq(httpReq, httpHeaders, payload, reqUrl, maskInfo);
			...
			TsmpApiLogReq dgrcPostRawDgrReqVo = getCommForwardProcService().addEsTsmpApiLogReq1(uuid, httpReq, maskPayload,
					"dgrc", aType);
			// 第一組 RDB Req
			TsmpApiLogReq dgrcPostRawDgrReqVo_rdb = getCommForwardProcService().addRdbTsmpApiLogReq1(uuid, httpReq,
					maskPayload, "dgrc", aType);
			...
			if (CollectionUtils.isEmpty(srcUrlList)) {
				...
				getCommForwardProcService().addRdbTsmpApiLogResp1(srcUrlListErrResp, dgrcPostRawDgrReqVo_rdb,
						respMbody);
				responseHandler
						.response(httpRes)
			...
			if (isMockTest) {
                ...
                mockForwardTo(httpReq, httpRes, httpHeaders, srcUrl, uuid, tokenPayload, dgrcPostRawDgrReqVo,
						dgrcPostRawDgrReqVo_rdb, cApiKeySwitch, apiReg);
//				responseHandler
//						.response(httpRes)
//						.handle(mockResp)
//						.write();
			...
			Map<String, Object> convertResponseBodyMap = forwardToByPolicy(httpHeaders, httpReq, httpRes, apiReg, uuid,
					tokenPayload, cApiKeySwitch, dgrcPostRawDgrReqVo, dgrcPostRawDgrReqVo_rdb, srcUrlList, payload, apiReg, maskInfo);
            ...
            if (!isSse) {
                ...
                // 第一組RDB RESP
                getCommForwardProcService().addRdbTsmpApiLogResp1(httpRes, dgrcPostRawDgrReqVo_rdb, httpRespStr,
                        content_Length);
            }
		} catch (Throwable e) {
	...
	private Map<String, Object> forwardToByPolicy(HttpHeaders httpHeaders, HttpServletRequest httpReq,
			...
			String payload, TsmpApiReg tsmpApiReg, Map<String, String> maskInfo) throws Exception {
		// 失敗判定策略
		...
		if ("1".equals(failHandlePolicy)) {// 1: 當調用目標URL失敗時,自動重試下一個目標URL
			...
			for (int i = 0; i < srcUrlList.size(); i++) {
				...
				convertResponseBodyMap = forwardTo(httpReq, httpRes, httpHeaders, payload, srcUrl, uuid, tokenPayload,
						dgrcPostRawDgrReqVo, dgrcPostRawDgrReqVo_rdb, cApiKeySwitch, tryNumWord, tsmpApiReg, maskInfo);
			...
			TPILogger.tl.debug("Src Url:" + srcUrl);
			convertResponseBodyMap = forwardTo(httpReq, httpRes, httpHeaders, payload, srcUrl, uuid, tokenPayload,
					dgrcPostRawDgrReqVo, dgrcPostRawDgrReqVo_rdb, cApiKeySwitch, null, tsmpApiReg, maskInfo);
		}
	...
	protected Map<String, Object> forwardTo(HttpServletRequest httpReq, HttpServletResponse httpRes,
											...
											String tryNumWord, TsmpApiReg tsmpApiReg, Map<String, String> maskInfo) throws Exception {
		...
		String autoCacheId = getCommForwardProcService().getAutoCacheIdByFlagStart(dgrcPostRawDgrReqVo, srcUrl,
				reqMbody);
		String fixedCacheId = getCommForwardProcService().getFixedCacheIdByFlagStart(dgrcPostRawDgrReqVo, srcUrl,
				reqMbody);
		if (StringUtils.hasText(autoCacheId)) {// 自適應cache
			...
			AutoCacheRespVo apiCacheRespVo = getProxyMethodServiceCacheProxy().queryByIdCallApi(autoCacheId, this, paramVo, maskInfo);
	...
	public HttpRespData callback(AutoCacheParamVo vo, Map<String, String> maskInfo) {
		try {
			...
			//第二組ES REQ
			String maskPayload = getCommForwardProcService().maskBody(maskInfo, vo.getReqMbody());
			TsmpApiLogReq dgrcPostRawBackendReqVo = getCommForwardProcService().addEsTsmpApiLogReq2(vo.getDgrReqVo(), vo.getHeader(), vo.getSrcUrl(), maskPayload);

			HttpRespData respObj = getHttpRespData(vo.getHttpMethod(), vo.getHeader(), vo.getSrcUrl(), vo.getReqMbody() , vo.getHttpReq(), null, maskInfo);
	...
	private HttpRespData callForwardApi(Map<String, List<String>> header, HttpServletRequest httpReq,
										...
										boolean isFixedCache, String tryNumWord, Map<String, String> maskInfo) throws Exception {
		...
		HttpRespData respObj = getHttpRespData(httpReq.getMethod(), header, srcUrl, payload,httpReq,httpRes.getOutputStream(),maskInfo);
...
//		rsMap.put("respHeader", respObj.respHeader);
	...
	protected HttpRespData getHttpRespData(String httpMethod, Map<String, List<String>> header,
										   String reqUrl, String payload, HttpServletRequest httpReq, ServletOutputStream sseOutput, Map<String, String> maskInfo) throws Exception {

		HttpRespData dgrcPostRaw_httpRespData = null;
		Pair<Boolean, CertificateInfo> pair = new Pair<>(false, null);
		DgrProtocol dgrProtocol = DgrProtocol.parse(reqUrl);
		if (!dgrProtocol.mtls()) {
			var targetUrl = dgrProtocol.getParsed(sslEnabled, serverPort);
			dgrcPostRaw_httpRespData = HttpUtil.httpReqByRawDataList(targetUrl, httpMethod, payload, header, true, false, maskInfo ,sseOutput);
		} else {
			...
			dgrcPostRaw_httpRespData =  HttpUtil2.callApiByHttp2(httpReq,reqUrl,  false, pair.getA(), pair.getB(), sseOutput,false);
		}
