...
public class TSMPCServicePatch {
	...
	public ResponseEntity<?> forwardToPatch(HttpHeaders httpHeaders, HttpServletRequest httpReq,
		...
		if (isMockTest) {
			return mockForwardTo(httpReq, httpRes, httpHeaders, tsmpcPatch_srcUrl, uuid, tokenPayload,
					tsmpcPatchDgrReqVo, tsmpcPatchDgrReqVo_rdb, cApiKeySwitch, apiReg);
		} else {
			return forwardTo(httpReq, httpRes, httpHeaders, payload, tsmpcPatch_srcUrl, uuid, tokenPayload,
					tsmpcPatchDgrReqVo, tsmpcPatchDgrReqVo_rdb, cApiKeySwitch, apiReg);
		}
	}
	...
	protected ResponseEntity<?> forwardTo(HttpServletRequest httpReq, HttpServletResponse httpRes,
			...
			TsmpApiLogReq tsmpcPatchDgrReqVo, TsmpApiLogReq tsmpcPatchDgrReqVo_rdb, Boolean cApiKeySwitch, TsmpApiReg tsmpApiReg)
			throws Exception {
		...
		Optional.ofNullable(respObj.loggerElapsedTimeMsg(uuid, reqUrl , respObj.respStr)).ifPresent(TPILogger.tl::warn);
	...
	private StringBuffer getLogResp(HttpServletResponse httpRes, String httpRespStr, int content_Length)
			throws IOException {
		StringBuffer log = new StringBuffer();

		// print header
		writeLogger(log, "--【Http Resp Header】--");
		List<String> headerName = httpRes.getHeaderNames().stream().distinct().collect(Collectors.toList());// 移除重複的
																											// HeaderName
