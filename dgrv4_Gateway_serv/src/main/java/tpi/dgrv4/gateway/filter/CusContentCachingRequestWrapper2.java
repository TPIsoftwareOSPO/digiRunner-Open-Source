...
/**
 * <p>
 * The reason is that ContentCachingRequestWrapper is hardcoded to only accept x-www-form-urlencoded for POST requests,
 * but our requirement needs to include PUT and PATCH as well.
 * For easier management,
 * we prefer not to use reflection and instead use a copied and modified approach.
 *
 *<p/>
 * {@link ContentCachingRequestWrapper} is a wrapper that caches the request body.
 */
public class CusContentCachingRequestWrapper2 extends HttpServletRequestWrapper {
     ...
     */
    public CusContentCachingRequestWrapper2(HttpServletRequest request) {
        super(request);
     ...
     * @since 4.3.6
     */
    public CusContentCachingRequestWrapper2(HttpServletRequest request, int contentCacheLimit) {
        super(request);
        int contentLength = request.getContentLength();
        if (contentLength > 0) {
    ...
    public ServletInputStream getInputStream() throws IOException {
        if (this.inputStream == null) {
            this.inputStream = new CusContentCachingRequestWrapper2.ContentCachingInputStream(getRequest().getInputStream());
        }
    ...
    public String getParameter(String name) {
        if (this.cachedContent.size() == 0 && isForm()) {
    ...
    public void writeRequestParametersToCachedContent() {
        try {
     ...
     *
     * @see #CusContentCachingRequestWrapper2(HttpServletRequest, int)
     */
