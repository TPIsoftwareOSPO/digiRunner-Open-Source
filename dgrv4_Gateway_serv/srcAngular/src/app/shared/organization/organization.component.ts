...
export class OrganizationComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.orgs = orgChart.find((org) => org.data.master == true)
      ? [orgChart.find((org) => org.data.master == true)!]
      : [];
  ...
  ngAfterViewInit() {
    if (window.location.hash == '#/ac00/ac0002') {
      // 建立使用者時， 依據所屬組織，限制只能點選包含自己以下的組織
      ...
      window.setTimeout(() => {
        ...
        let table = document.getElementById(
          this._orgList?.find((org) => org.orgID == this.tool.getOrgId())
            ?.orgName +
            '_' +
            this.uuid
        );
        ...
        for (
          let i = 0;
          i < table!.getElementsByClassName('ui-treenode-selectable').length;
          i++
        ) {
          table
            ?.getElementsByClassName('ui-treenode-selectable')
            ?.item(i)
            ?.classList.remove('ui-cursor');
        }
      });
    }
  }
  ...
  filter() {
    if (
      ...
      this.form.get('orgName')!.value != this.currentHighlight
    ) {
    ...
    if (
      document.getElementById(this.form.get('orgName')!.value + '_' + this.uuid)
    ) {
      this.currentHighlight = this.form.get('orgName')!.value + '_' + this.uuid;

      document.getElementById(
        this.form.get('orgName')!.value + '_' + this.uuid
      )!.parentElement!.parentElement!.parentElement!.style.backgroundColor =
        'pink';

      document
        .getElementById(this.form.get('orgName')!.value + '_' + this.uuid)
        ?.focus();
      // this.form.reset();
    } else {
      if (document.getElementById(this.currentHighlight)) {
        document.getElementById(
          ...
          '';
        document.getElementById(this.currentHighlight)!.parentElement
  ...
  onNodeSelect(node) {
    ...
    this._selectedNode.emit(node.node.data);
  }
