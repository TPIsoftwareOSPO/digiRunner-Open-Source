...
export class TokenService {
  ...
  auth(
    username: string,
    mima: string,
    grant_type: GrantType = GrantType.mima,
    appendUrl: string = ''
  ) {
    try {
      ...
      return this.http
        .post<ResToken>(url, body, { headers: this.headers })
        .pipe(catchError(this.handleError.bind(this)));
      // return of({
  ...
  public authBytestApi(
    ...
    try {
      ...
      return this.http
        .post<ResToken>(`${environment.apiUrl}/${this.testApiPath}`, body, {
          headers: testHeader,
        })
        .pipe(catchError(this.handleError.bind(this)));
    } catch (error) {
  ...
  logout() {
    const logoutUrl: string | undefined = sessionStorage.getItem('AcConf')
      ? (JSON.parse(sessionStorage.getItem('AcConf')!) as AA0510Resp).logoutUrl
      : undefined;
