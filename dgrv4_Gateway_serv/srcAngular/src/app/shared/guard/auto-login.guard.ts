...
export class AutoLoginGuard implements CanActivate {
  ...
  private get signBlockService():SignBlockService{
     return this.injector.get(SignBlockService);
  }
  ...
  canActivate(
    ...
    return new Observable((obser) => {
      ...
      if (sso != undefined && sso == 'true') {
        ...
        this.tokenService
          .auth('', '', GrantType.mima, appendUrl.join('&'))
                ...
                if (this.toolService.checkSuccess(r.ResHeader)) {
                  ...
                  this.util.getAcConf().subscribe((res) => {
                    if (this.toolService.checkDpSuccess(res.ResHeader)) {
                      this.toolService.writeAcConf(
                        JSON.stringify(res.RespBody)
                      );
                      //check由按鈕超連結的頁面是否有權限
                      let checks = [
                        'AC0004',
