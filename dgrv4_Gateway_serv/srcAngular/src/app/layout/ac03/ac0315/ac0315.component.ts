...
import { AlertType, ApiSrc } from 'src/app/models/common.enum';
...
export class Ac0315Component extends BaseComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.list
      .querySubItemsByItemNo(ReqBody_API_JWT_FLAG)
  ...
  moreData() {
    let ReqBody = {
      ...
      apiSrc: ['C'],
      paging: 'Y',
      moduleName: this.apilist_data[this.apilist_data.length - 1].moduleName.t
        ? this.apilist_data[this.apilist_data.length - 1].moduleName.ori
        : this.apilist_data[this.apilist_data.length - 1].moduleName.val,
      apiKey: this.apilist_data[this.apilist_data.length - 1].apiKey.t
        ? this.apilist_data[this.apilist_data.length - 1].apiKey.ori
        : this.apilist_data[this.apilist_data.length - 1].apiKey.val,
    } as AA0301Req;
    this.apiService.queryAPIList_v3(ReqBody).subscribe((res) => {
      if (this.tool.checkDpSuccess(res.ResHeader)) {
  ...
  choose_copy_api() {
    ...
    this.form
      .get('copy_target')!
      .setValue(
        `${this.selected_page2['moduleName']['val']}-${this.selected_page2['apiName']['val']}(${this.selected_page2['apiKey']['val']})`,
      );
  }
  ...
  copy() {
    ...
    this.clear();
    let detailReqBody = {
      moduleName: this.copy_target.moduleName.ori
        ? this.copy_target.moduleName.ori
        : this.copy_target.moduleName.val,
      apiKey: this.copy_target.apiKey.ori
        ? this.copy_target.apiKey.ori
        : this.copy_target.apiKey.val,
    } as AA0302Req;
    this.apiService.queryAPIDetail_v3(detailReqBody).subscribe((res) => {
      if (this.tool.checkDpSuccess(res.ResHeader)) {
        console.log(res.RespBody);
        this.apiDetail = res.RespBody;
        const _moduleName = res.RespBody.moduleName.o
          ? res.RespBody.moduleName.o
          : res.RespBody.moduleName.v;
        const _apiKey = res.RespBody.apiKey.o
        ...
        this.form
          .get('urlRID')!
          .setValue(res.RespBody.urlRID == '1' ? true : false);
        this.form
          .get('no_oauth')!
          .setValue(res.RespBody.noOAuth == '1' ? true : false);
        this.form
          .get('tokenPayload')!
          .setValue(res.RespBody.funFlag == 1 ? true : false);
        if (
          (res.RespBody.jweFlag.v != '0' && res.RespBody.jweFlag.v != null) ||
          (res.RespBody.jweFlagResp.v != '0' &&
            res.RespBody.jweFlagResp.v != null)
        ) {
  ...
  async composer() {
    ...
    let req = {
      ...
      dataFormat: this.translateStr(this.form.get('dataFormat')!.value),
      jweFlag: this.translateStr(this.form.get('jweFlag')!.value),
      jweFlagResp: this.translateStr(this.form.get('jweFlagResp')!.value),
      labelList: this.labelList.value,
    } as AA0311Req;
    if (JSON.stringify(this.apiDetail) != '{}' && this.apiDetail) {
      req.consumes = this.apiDetail.consumesOfJson?.t
        ? JSON.parse(this.apiDetail.consumesOfJson.o ?? '')
        : JSON.parse(this.apiDetail.consumesOfJson!.v);
      req.produces = this.apiDetail.producesOfJson?.t
        ? JSON.parse(this.apiDetail.producesOfJson.o ?? '')
        : JSON.parse(this.apiDetail.producesOfJson!.v);
      req.headers = this.apiDetail.headersOfJson?.t
        ? JSON.parse(this.apiDetail.headersOfJson.o ?? '')
        : JSON.parse(this.apiDetail.headersOfJson!.v);
      req.params = this.apiDetail.paramsOfJson?.t
      ...
      if (this.tool.checkDpSuccess(res.ResHeader)) {
        getAuthCode$.subscribe(async (res) => {
          if (this.tool.checkDpSuccess(res.ResHeader)) {
            ...
            this.alert.ok(
              '',
              `${dict['message.create']} ${dict['message.success']}`,
            );
          }
        });
  ...
  funFlagConvert(): Object {
    return {
      tokenPayload: this.form.get('tokenPayload')!.value ? true : false,
    };
  }
