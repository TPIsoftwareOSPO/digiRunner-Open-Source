...
export class Ac0006Component extends BaseComponent implements OnInit {
  ...
  constructor(
    ...
    this.form
      .get('newUserBlock')
      ?.setValidators([
        ValidatorFns.confirmPasswordForUserValidator(this.form, true),
        ValidatorFns.maxLengthValidator(this.newUserBlockLimitChar.value),
      ]);
    this.form
      .get('confirmUserBlock')
      ?.setValidators([
        ValidatorFns.confirmPasswordForUserValidator(this.form, true),
        ValidatorFns.maxLengthValidator(this.newUserBlockLimitChar.value),
      ]);
    ...
    //     if(this.mimaPattern) {
    //       this.form.get("newUserBlock")?.addValidators([ValidatorFns.patternValidator(this.mimaPattern!, this.mimatip)])
    //     }
    //   }
    // });
  }
  ...
  ngOnInit() {
    ...
    this.userService.getMimaStrengthDesc().subscribe((res) => {
      if (this.tool.checkDpSuccess(res.ResHeader)) {
        ...
        this.form
          .get('newUserBlock')
          ?.addValidators(ValidatorFns.minLengthValidator(6, this.mimatip));
  ...
  submitForm(formDirective: FormGroupDirective) {
    ...
    this.userService.updateTUserData(ReqBody).subscribe(async (res) => {
      if (this.tool.checkDpSuccess(res.ResHeader)) {
        ...
        this.message.add({
          severity: 'success',
          summary: `${dict['message.update']} ${dict['profile']}`,
          detail: `${dict['message.update']} ${dict['message.success']}!`,
        });
        if (
          (ReqBody.newUserName != '' &&
            ReqBody.newUserName != this.user_name) ||
          (ReqBody.newUserBlock != '' &&
            ReqBody.userBlock != ReqBody.newUserBlock)
        ) {
          this.alert.logout(
  ...
  public get newUserBlock() {
    return this.form.get('newUserBlock');
  }
