import {
  AA0201HostReq,
  AA0201HostReqAddNo,
} from './../../../../models/api/ClientService/aa0201.interface';
import {
  Component,
  OnInit,
  ViewChild,
...
export class HostInputComponent implements OnInit, ControlValueAccessor {
  ...
  addHostInput(newhost?: AA0201HostReq) {
    ...
    let componentRef = this.hostInputRef.createComponent(
      HostInputDetailComponent
    );
    if (newhost) {
      this.hosts.push({
        hostName: newhost.hostName,
        hostIP: newhost.hostIP,
        no: this.hostnums,
      });
    } else {
    ...
    if (this.onChange)
      this.onChange(
        this.hosts.map((item) => {
          return { hostName: item.hostName, hostIP: item.hostIP };
        })
      );
