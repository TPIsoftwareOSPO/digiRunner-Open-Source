...
import {
  AA0202List,
  AA0202Req,
} from '../../../models/api/ClientService/aa0202.interface';
...
import {
  ...
  AA0203Resp,
  AA0203VgroupInfo,
} from '../../../models/api/ClientService/aa0203.interface';
...
import {
  AA0201HostReq,
  AA0201Req,
} from 'src/app/models/api/ClientService/aa0201.interface';
...
import {
  AA0228GroupInfo,
  AA0228Req,
} from 'src/app/models/api/ClientService/aa0228.interface';
...
import {
  AA0229Req,
  AA0229VgroupInfo,
} from 'src/app/models/api/ClientService/aa0229.interface';
...
import {
  AA0238Req,
  GroupInfo_0238,
} from 'src/app/models/api/ClientService/aa0238.interface';
import { of } from 'rxjs';
...
export class Ac0202Component extends BaseComponent implements OnInit {
  ...
  async getDicts() {
    return await this.translate
      .get([
        ...
        'security_level',
        'status',
        'dialog.client_update',
        'dialog.client_delete',
        'dialog.client_security_setting',
        'message.create',
        'message.client',
        'message.setting',
        'message.security',
        ...
        'virtul_group_desc',
        'cfm_del_client',
        'grant_type_options.authorization_code',
        'grant_type_options.client_credentials',
        'grant_type_options.refresh_token',
        'grant_type_options.password',
        'grant_type_options.implicit',
        'grant_type_options.public',
        'token_exp_time_options.day',
        'token_exp_time_options.hour',
        'token_exp_time_options.minute',
        'token_exp_time_options.second',
        'grant_type_options.smsotp',
        'grant_type_options.group',
        'group_name',
        'virtul_group_name',
        'button.manager_oauth_group',
        'button.manager_oauth_virtual_group',
        'api_scope_desc',
        'date_range_required_if_exist',
        'message.service_time_error',
        'sdate_over_edate',
        'stime_over_etime',
        'timezone_required',
  ...
  async ngOnInit() {
    ...
    this.list.querySubItemsByItemNo(ReqBody).subscribe((res) => {
      if (this.toolService.checkDpSuccess(res.ResHeader)) {
        ...
        res.RespBody.subItems?.map((item) => {
          _queryStatusOpt.push({
            label: item.subitemName,
            value: item.subitemNo,
          });
          if (item.subitemNo != '-1' && item.subitemNo != '2') {
            // 只留啟用、停用
            _createStatusOpt.push({
              label: item.subitemName,
              value: item.param1 ?? '',
            });
          }
    ...
    let publicFlagReqBody = {
      encodeItemNo:
        this.toolService.Base64Encoder(
          this.toolService.BcryptEncoder('API_AUTHORITY')
        ) +
        ',' +
        7,
      isDefault: 'N',
    ...
    for (let i = 0; i < 10; i++) {
      ...
      if (i == 0) {
        key = i + ' (high) ';
        this.priorities.push({ label: key, value: i });
      }
      if (i == 5) {
        key = i + ' (default) ';
        this.priorities.push({ label: key, value: i });
    ...
    this.roleService
      .queryRTMapByUk({
        txIdList: ['AA0202', 'AA0201', 'AA0203', 'AA0204', 'AA0205'],
      } as DPB0115Req)
      .subscribe((res) => {
        if (this.toolService.checkDpSuccess(res.ResHeader)) {
          this.canQuery = res.RespBody.dataList.find(
            (item) => item.txId === 'AA0202'
          )
            ? res.RespBody.dataList.find((item) => item.txId === 'AA0202')!
                .available
            : false;
          this.canCreate = res.RespBody.dataList.find(
            (item) => item.txId === 'AA0201'
          )
            ? res.RespBody.dataList.find((item) => item.txId === 'AA0201')!
                .available
            : false;
          this.canDetail = res.RespBody.dataList.find(
            (item) => item.txId === 'AA0203'
          )
            ? res.RespBody.dataList.find((item) => item.txId === 'AA0203')!
                .available
            : false;
          this.canUpdate = res.RespBody.dataList.find(
            (item) => item.txId === 'AA0204'
          )
            ? res.RespBody.dataList.find((item) => item.txId === 'AA0204')!
                .available
            : false;
          // this.canSecurity = res.RespBody.dataList.find(item => item.txId === 'DPB0099').available;
          this.canDelete = res.RespBody.dataList.find(
            (item) => item.txId === 'AA0205'
          )
            ? res.RespBody.dataList.find((item) => item.txId === 'AA0205')!
                .available
            : false;
        }
      });
    ...
    let tokenExpiryTimeOptReqBody = {
      encodeItemNo:
        this.toolService.Base64Encoder(
          this.toolService.BcryptEncoder('TIME_UNIT')
        ) +
        ',' +
        25,
      isDefault: 'N',
    ...
    this.list
      .querySubItemsByItemNo(tokenExpiryTimeOptReqBody)
      .subscribe((res) => {
        if (this.toolService.checkDpSuccess(res.ResHeader)) {
          let _tokenExpiryTimes: { label: string; value: string }[] = [];
          res.RespBody.subItems?.map((item) => {
            _tokenExpiryTimes.push({
              label: item.subitemName,
              value: item.subitemNo,
            });
          });
    ...
    this.userService.getMimaStrengthDesc().subscribe((res) => {
      if (this.toolService.checkDpSuccess(res.ResHeader)) {
        this.mimatip = res.RespBody.clientPwdStrengthDesc;
        this.mimaPattern = res.RespBody.clientPwdStrength;
      }
    });
  }
  ...
  async init() {
    ...
    this.groupListDataCols = [
      {
        field: 'groupName',
        header: `${dicts['group_name']} - (${dicts['group_alias']})`,
      },
    ...
    this.clientVGroupListCols = [
      {
        field: 'vgroupName',
        header: `${dicts['virtul_group_name']} - (${dicts['virtul_group_alias']})`,
      },
    ...
    this.grantTypeOptions = [
      {
        label: dicts['grant_type_options.authorization_code'],
        value: 'authorization_code',
      },
      {
        label: dicts['grant_type_options.client_credentials'],
        value: 'client_credentials',
      },
      {
        label: dicts['grant_type_options.refresh_token'],
        value: 'refresh_token',
      },
    ...
    let ReqBody = {
      ...
      encodeStatus:
        ...
        ',' +
        this.convertEncodeStatusIndex(this.encodeStatus!.value, 'query'),
    } as AA0202Req;
  ...
  moreGroupListData() {
    ...
    this.clientService.queryGroupList_0238(ReqBody).subscribe((res) => {
      if (this.toolService.checkDpSuccess(res.ResHeader)) {
        this.groupListData = this.groupListData.concat(
          res.RespBody.groupInfoList
        );
        this.groupListDataRowcount = this.groupListData.length;
      }
    });
  ...
  async createClientData() {
    ...
    let ReqBody = {
      clientID:
        this.clientID!.value != ''
          ? this.clientID!.value
          : this.toolService.Base64Encoder(this.clientName!.value),
      ...
      encodeStatus:
        ...
        ',' +
        this.convertEncodeStatusIndex(this.createStatus!.value, 'create'),
      publicFlag:
        this.toolService.Base64Encoder(
          this.toolService.BcryptEncoder(this.publicFlag!.value)
        ) +
        ',' +
    ...
    if (
      !this.isNullUndefinedNoneCheck(this.clientStartDate!.value) ||
      !this.isNullUndefinedNoneCheck(this.clientEndDate!.value)
    ) {
      if (
        this.isNullUndefinedNoneCheck(this.clientStartDate!.value) ||
        this.isNullUndefinedNoneCheck(this.clientEndDate!.value)
      ...
      if (
        new Date(this.clientEndDate!.value).getTime() <
        new Date(this.clientStartDate!.value).getTime()
      ) {
    ...
    if (
      !isNaN(this.svcST!.value.hour) ||
      !isNaN(this.svcST!.value.minute) ||
      !isNaN(this.svcET!.value.hour) ||
      !isNaN(this.svcET!.value.minute)
    ) {
      if (
        isNaN(this.svcST!.value.hour) ||
        isNaN(this.svcST!.value.minute) ||
    ...
    this.clientService.addClient(ReqBody).subscribe((res) => {
      if (this.toolService.checkDpSuccess(res.ResHeader)) {
        this.messageService.add({
          severity: 'success',
          summary: `${dicts['message.create']} ${dicts['message.client']}`,
          detail: `${dicts['message.create']} ${dicts['message.success']}!`,
        });
        this.encodeStatus!.setValue('-1');
        this.submitForm();
  ...
  async updateClientData() {
    ...
    let ReqBody = {
      ...
      newApiQuota: this.newApiQuota!.value
        ? this.newApiQuota!.value.toString()
        : '',
      ...
      newEncodePublicFlag:
        this.toolService.Base64Encoder(
          this.toolService.BcryptEncoder(this.newEncodePublicFlag!.value)
        ) +
        ',' +
        this.newEncodePublicFlag!.value,
    ...
    if (
      !this.isNullUndefinedNoneCheck(this.newClientStartDate!.value) ||
      !this.isNullUndefinedNoneCheck(this.newClientEndDate!.value) ||
      !isNaN(this.newSvcST!.value.hour) ||
      !isNaN(this.newSvcST!.value.minute) ||
      !isNaN(this.newSvcET!.value.hour) ||
      !isNaN(this.newSvcET!.value.minute)
    ) {
    ...
    if (
      ...
      if (
        new Date(this.newClientEndDate!.value).getTime() <
        new Date(this.newClientStartDate!.value).getTime()
      ) {
        this.alertService.ok(dicts['sdate_over_edate'], '');
        return;
      }

      ReqBody.newClientStartDate = dayjs
        .utc(dayjs.tz(this.newClientStartDate!.value, this.newTimeZone.value))
        .format();
      ReqBody.newClientEndDate = dayjs
        .utc(dayjs.tz(this.newClientEndDate!.value, this.newTimeZone.value))
        .format();
    } else {
    ...
    if (
      ...
      ReqBody.newClientStartTimePerDay = dayjs
        .utc(dayjs.tz(sTime, this.newTimeZone.value))
        .format();
      ReqBody.newClientEndTimePerDay = dayjs
        .utc(dayjs.tz(eTime, this.newTimeZone.value))
        .format();
    } else {
    ...
    this.clientService.updateClient(ReqBody).subscribe((res) => {
      if (this.toolService.checkDpSuccess(res.ResHeader)) {
        this.messageService.add({
          severity: 'success',
          summary: `${dicts['message.update']} ${dicts['message.client']}`,
          detail: `${dicts['message.update']} ${dicts['message.success']}!`,
        });
        this.encodeStatus!.setValue('-1');
        this.submitForm();
        this.changePage('query');
  ...
  async deleteClientData() {
    ...
    this.confirmationService.confirm({
      ...
      message: `${dicts['client_id']}:${this.clientDetail!.clientID}，${
        dicts['client_name']
  ...
  async onDeleteClientConfirm() {
    ...
    this.clientService.deleteClientByClientId(ReqBody).subscribe((res) => {
      if (this.toolService.checkDpSuccess(res.ResHeader)) {
        this.messageService.add({
          severity: 'success',
          summary: `${dicts['message.delete']} ${dicts['message.client']}`,
  ...
  moreGroupList() {
    ...
    this.clientService.queryGroupList_v3(ReqBody).subscribe((res) => {
      if (this.toolService.checkDpSuccess(res.ResHeader)) {
        this.groupInfoList = this.groupInfoList.concat(
          res.RespBody.groupInfoList
        );
        this.groupInfoListRowcount = this.groupInfoList.length;
      }
  ...
  async deleteClientGroup(rowData: AA0228GroupInfo) {
    ...
    this.confirmationService.confirm({
      ...
      message: `${dicts['client_name']}:${this.clientDetail!.clientName}，${
        dicts['group_name']
      }:${this.currentDeleteGroup.groupName}`,
      accept: () => {
  ...
  moreVGroupList() {
    ...
    this.clientService.queryVGroupList(ReqBody).subscribe((res) => {
      if (this.toolService.checkDpSuccess(res.ResHeader)) {
        this.vgroupInfoList = this.vgroupInfoList.concat(
          res.RespBody.vgroupInfoList
        );
        this.vgroupInfoListRowcount = this.vgroupInfoList.length;
      }
    });
  ...
  updateClientVGroup() {
    ...
    this.clientService
      .addClientVGroupByClientId(ReqBody)
      .subscribe(async (res) => {
        if (this.toolService.checkDpSuccess(res.ResHeader)) {
          this.refreshClientDetail();
          // this.changePage('security', this.currentClient);
  ...
  async deleteClientVGroup(rowData: AA0229VgroupInfo) {
    ...
    this.confirmationService.confirm({
      ...
      message: `${dicts['client_name']}:${this.clientDetail!.clientName}，${
        dicts['virtul_group_name']
      }:${this.currentDeleteVGroup.vgroupName}`,
      accept: () => {
  ...
  onDeleteClientVGroup() {
    ...
    this.clientService
      .deleteClientVGroupByClientId(ReqBody)
      .subscribe((res) => {
        if (this.toolService.checkDpSuccess(res.ResHeader)) {
          this.refreshClientDetail();
        }
  ...
  getTokenSetting() {
    ...
    this.clientService.getTokenSettingByClient(ReqBody).subscribe((res) => {
      if (this.toolService.checkDpSuccess(res.ResHeader)) {
        ...
        this.convertTokenExpireTimes(
          'access',
          res.RespBody.accessTokenValidity
        );
        this.convertTokenExpireTimes(
          'refresh',
          res.RespBody.raccessTokenValidity!
        );
  ...
  updateClientToken() {
    let ReqBody = {
      ...
      accessTokenValidityTimeUnit:
        this.toolService.Base64Encoder(
          this.toolService.BcryptEncoder(
            this.accessTokenValidityTimeUnit!.value
          )
        ) +
        ',' +
        this.convertTokenValidityTimeUnitIndex(
          this.accessTokenValidityTimeUnit!.value
        ),
      raccessTokenValidityTimeUnit:
        this.toolService.Base64Encoder(
          this.toolService.BcryptEncoder(
            this.raccessTokenValidityTimeUnit!.value
          )
        ) +
        ',' +
        this.convertTokenValidityTimeUnitIndex(
    ...
    if (this.webServerRedirectUri1?.value)
      ReqBody.webServerRedirectUri1 = this.webServerRedirectUri1?.value;
    if (this.webServerRedirectUri2?.value)
      ReqBody.webServerRedirectUri2 = this.webServerRedirectUri2?.value;
    if (this.webServerRedirectUri3?.value)
      ReqBody.webServerRedirectUri3 = this.webServerRedirectUri3?.value;
    if (this.webServerRedirectUri4?.value)
      ReqBody.webServerRedirectUri4 = this.webServerRedirectUri4?.value;
    if (this.webServerRedirectUri5?.value)
      ReqBody.webServerRedirectUri5 = this.webServerRedirectUri5?.value;

    this.clientService
      .updateTokenSettingByClient(ReqBody)
      .subscribe(async (res) => {
        if (this.toolService.checkDpSuccess(res.ResHeader)) {
          const code = [
            'message.update',
          ...
          this.messageService.add({
            severity: 'success',
            summary: `${dict['message.update']} ${dict['dialog.client_security_setting']}`,
  ...
  updateStatusSetting() {
    let ReqBody = {
      ...
      encodeStatus:
        this.toolService.Base64Encoder(
          this.toolService.BcryptEncoder(this.updateStatus!.value)
        ) +
        ',' +
        this.convertEncodeStatusIndex(this.updateStatus!.value, 'update'),
    } as AA0220Req;
  ...
  updateClientPwd() {
    let ReqBody = {
      ...
      clientBlock: this.toolService.Base64Encoder(this.clientBlock!.value),
      newClientBlock: this.toolService.Base64Encoder(
        this.newClientBlock!.value
      ),
      confirmNewClientBlock: this.toolService.Base64Encoder(
        this.confirmNewClientBlock!.value
      ),
    } as AA0231Req;
    this.clientService
      .updatePasswordSettingByClient(ReqBody)
      .subscribe((res) => {
        if (this.toolService.checkDpSuccess(res.ResHeader)) {
  ...
  refreshClientDetail() {
    ...
    if (this.newSecurityID!.value) {
      this.securityLevelOptions.filter(
        (opt) => opt.value == this.newSecurityID!.value
      )[0].disabled = true;
    }
    let ReqBody = {
      clientID: this.clientDetail!.clientID,
  ...
  changeTab(evn) {
    ...
    switch (evn.index) {
      ...
      case 4: // x api key setting
        ...
        break;
      case 5: // 狀態管理
        this.resetFailLoginTreshhold!.setValue(
          this.clientDetail!.failTreshhold
        );
        this.updateStatus!.setValue(this.clientDetail!.status);
        this.resetPwdFailTimes!.setValue(false);
        ...
        break;
      case 6: // 密碼管理
        this.addPasswordFormValidator(evn.index);
  ...
  async changePage(action: string, rowData?: AA0202List) {
    ...
    switch (action) {
      case 'create':
        this.clientService.addClient_before().subscribe((res) => {
          if (this.toolService.checkDpSuccess(res.ResHeader)) {
            ...
            this.pageNum = 2;
            this.clientName!.setValidators([
              ValidatorFns.requiredValidator(),
              ValidatorFns.stringNameValidator(this.maxlength50.value),
            ]);
            this.clientAlias!.setValidators([
              ValidatorFns.requiredValidator(),
              ValidatorFns.maxLengthValidator(100),
            ]);
            this.signupNum!.setValidators(
              ValidatorFns.maxLengthValidator(this.maxlength100.value)
            );
            this.clientBlock!.setValidators([
              ValidatorFns.confirmPasswordForClientValidator(this.form),
              ValidatorFns.requiredValidator(),
              ValidatorFns.maxLengthValidator(this.maxlength256.value),
            ]);
            if (this.mimaPattern) {
              this.clientBlock?.addValidators([
            ...
            this.confirmClientBlock!.setValidators([
              ValidatorFns.confirmPasswordForClientValidator(this.form),
              ValidatorFns.requiredValidator(),
              ValidatorFns.maxLengthValidator(this.maxlength256.value),
            ]);
            this.owner!.setValidators([
              ValidatorFns.requiredValidator(),
              ValidatorFns.maxLengthValidator(this.maxlength50.value),
            ]);
            ...
            this.emails!.setValidators(
              ValidatorFns.maxLengthValidator(this.maxlength500.value)
            );
      ...
      case 'detail':
        ...
        this.clientService.queryClientDetail(detailReqBody).subscribe((res) => {
          if (this.toolService.checkDpSuccess(res.ResHeader)) {
            ...
            this.pageNum = 6;
            if (
              this.clientDetail.groupInfoList &&
              this.clientDetail.groupInfoList.length != 0
            ) {
              for (let groupInfo of this.clientDetail.groupInfoList) {
                this.clientDetailGroupList.push(
                  `${groupInfo.groupName} - (${
                    groupInfo.groupAlias ? groupInfo.groupAlias : ''
                  })`
                );
              }
            }
          ...
          if (this.toolService.checkDpSuccess(res.ResHeader)) {
            ...
            if (this.clientDetail.timeZone) {
              this.timeZoneList.push({
                label: this.timezoneOffsetFormat(this.clientDetail.timeZone),
                value: this.clientDetail.timeZone,
              });
            }
            ...
            if (this.clientDetail.clientStartDate) {
              let localSDate: string = this.getDateString(
                this.clientDetail.clientStartDate,
                this.newTimeZone.value
              );
            ...
            if (this.clientDetail.clientEndDate) {
              let localEDate: string = this.getDateString(
                this.clientDetail.clientEndDate,
                this.newTimeZone.value
              );
            ...
            if (this.clientDetail.clientStartTimePerDay) {
              let localsTime = this.getTimeString(
                this.clientDetail.clientStartTimePerDay,
                this.newTimeZone.value
              ).split(':');
              this.clientStartTimePerDay?.setValue(
                this.clientDetail.clientStartTimePerDay
              );

              this.newSvcST!.setValue({
                hour: localsTime[0],
                minute: localsTime[1],
              });
            }
            ...
            if (this.clientDetail.clientEndTimePerDay) {
              let localeTime = this.getTimeString(
                this.clientDetail.clientEndTimePerDay,
                this.newTimeZone.value
              ).split(':');
              this.clientEndTimePerDay?.setValue(
                this.clientDetail.clientEndTimePerDay
              );

              this.newSvcET!.setValue({
                hour: localeTime[0],
                minute: localeTime[1],
              });
            }
            ...
            if (dayjs.tz.guess() !== this.clientDetail.timeZone) {
              this.timeZoneList.push({
                label: this.timezoneOffsetFormat(dayjs.tz.guess()),
      ...
      case 'security':
        ...
        this.clientService
          .queryClientDetail(securityReqBody)
          .subscribe((res) => {
            if (this.toolService.checkDpSuccess(res.ResHeader)) {
              this.clientDetail = res.RespBody;
              ...
              this.groupAuthService.queryTSecurityLV({}).subscribe((res) => {
                if (this.toolService.checkDpSuccess(res.ResHeader)) {
                  ...
                  res.RespBody.securityLevelList?.map((scope) => {
                    if (
                      scope.securityLevelId ==
                      this.clientDetail?.securityLV.securityLevelId
                    ) {
                      _securityLevels.push({
                        label: scope.securityLevelName,
                        value: scope.securityLevelId,
                        disabled: true,
                      });
                    } else {
                      _securityLevels.push({
  ...
  // 密碼欄位檢查
  addPasswordFormValidator(idx) {
    this.clientService
      .updatePasswordSettingByClient_before()
      .subscribe((res) => {
        if (this.toolService.checkDpSuccess(res.ResHeader)) {
          this.addFormValidator(this.form, res.RespBody.constraints);
          ...
          this.form
            ...
            ?.setValidators([
              ValidatorFns.confirmPasswordValidator(
                this.form,
                'newClientBlock',
                'confirmNewClientBlock'
              ),
              ValidatorFns.maxLengthValidator(
                this.newClientBlockLimitChar.value
              ),
            ]);
          this.form
            .get('confirmNewClientBlock')
            ?.setValidators([
              ValidatorFns.confirmPasswordValidator(
                this.form,
                'newClientBlock',
          ...
          if (idx) {
            if (this.mimaPattern) {
              this.newClientBlock?.addValidators([
                ValidatorFns.patternValidator(
                  this.mimaPattern,
                  this.mimatip ?? this.mimaPattern
                ),
  ...
  removeGroup(selRowData: AA0228GroupInfo) {
    let filterData = this.groupIdList.value.filter(
      (rowdata: AA0228GroupInfo) => rowdata.groupID != selRowData.groupID
    );
  ...
  createXApiKey() {
    let req = {
      ...
      groupIdList: this.groupIdList.value.map(
        (rowData: AA0228GroupInfo) => rowData.groupID
      ),
    } as DPB0208Req;
    if (this.effectiveAt.value)
      req.effectiveAt = new Date(this.effectiveAt.value).getTime().toString();
  ...
  async deleteXApiKeyById(rowData: DPB0207RespItem) {
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.serverService
          .deleteXApiKey({ id: rowData.id })
          .subscribe(async (res) => {
            if (this.toolService.checkDpSuccess(res.ResHeader)) {
              this.messageService.add({
                severity: 'success',
                summary: `${dict['message.delete']} X-Api-Key`,
  ...
  public get signupNum() {
    return this.form.get('signupNum');
  }
  public get clientAlias() {
    return this.form.get('clientAlias');
  }
  public get clientBlock() {
    return this.form.get('clientBlock');
  }
  public get confirmClientBlock() {
    return this.form.get('confirmClientBlock');
  }
  public get hostList() {
    return this.form.get('hostList');
  }
  public get clientStartDate() {
    return this.form.get('clientStartDate');
  }
  public get clientEndDate() {
    return this.form.get('clientEndDate');
  }
  public get svcST() {
    return this.form.get('svcST');
  }
  public get svcET() {
    return this.form.get('svcET');
  }
  public get apiQuota() {
    return this.form.get('apiQuota');
  }
  public get tps() {
    return this.form.get('tps');
  }
  public get cPriority() {
    return this.form.get('cPriority');
  }
  public get owner() {
    return this.form.get('owner');
  }
  public get createStatus() {
    return this.form.get('createStatus');
  }
  public get publicFlag() {
    return this.form.get('publicFlag');
  }
  public get emails() {
    return this.form.get('emails');
  }
  public get newClientName() {
    return this.form.get('newClientName');
  }
  public get newClientAlias() {
    return this.form.get('newClientAlias');
  }
  public get newSignupNum() {
    return this.form.get('newSignupNum');
  }
  public get newHostList() {
    return this.form.get('newHostList');
  }
  public get newClientStartDate() {
    return this.form.get('newClientStartDate');
  }
  public get newClientEndDate() {
    return this.form.get('newClientEndDate');
  }
  public get newSvcST() {
    return this.form.get('newSvcST');
  }
  public get newSvcET() {
    return this.form.get('newSvcET');
  }
  public get newApiQuota() {
    return this.form.get('newApiQuota');
  }
  public get newTps() {
    return this.form.get('newTps');
  }
  public get newCPriority() {
    return this.form.get('newCPriority');
  }
  public get newOwner() {
    return this.form.get('newOwner');
  }
  public get newEncodePublicFlag() {
    return this.form.get('newEncodePublicFlag');
  }
  public get newEmails() {
    return this.form.get('newEmails');
  }
  public get newSecurityID() {
    return this.form.get('newSecurityID');
  }
  public get authorizedGrantType() {
    return this.form.get('authorizedGrantType');
  }
  public get accessTokenValidity() {
    return this.form.get('accessTokenValidity');
  }
  public get raccessTokenValidity() {
    return this.form.get('raccessTokenValidity');
  }
  public get webServerRedirectUri() {
    return this.form.get('webServerRedirectUri');
  }
  public get webServerRedirectUri1() {
    return this.form.get('webServerRedirectUri1');
  }
  public get webServerRedirectUri2() {
    return this.form.get('webServerRedirectUri2');
  }
  public get webServerRedirectUri3() {
    return this.form.get('webServerRedirectUri3');
  }
  public get webServerRedirectUri4() {
    return this.form.get('webServerRedirectUri4');
  }
  public get webServerRedirectUri5() {
    return this.form.get('webServerRedirectUri5');
  }
  public get accessTokenValidityTimeUnit() {
    return this.form.get('accessTokenValidityTimeUnit');
  }
  public get raccessTokenValidityTimeUnit() {
    return this.form.get('raccessTokenValidityTimeUnit');
  }
  public get updateStatus() {
    return this.form.get('updateStatus');
  }
  public get resetFailLoginTreshhold() {
    return this.form.get('resetFailLoginTreshhold');
  }
  public get resetPwdFailTimes() {
    return this.form.get('resetPwdFailTimes');
  }
  public get newClientBlock() {
    return this.form.get('newClientBlock');
  }
  public get resetBlock() {
    return this.form.get('resetBlock');
  }
  public get confirmNewClientBlock() {
    return this.form.get('confirmNewClientBlock');
  }
  public get accessTokenQuota() {
    return this.form.get('accessTokenQuota');
  }
  public get refreshTokenQuota() {
    return this.form.get('refreshTokenQuota');
  }
  public get remark() {
    return this.form.get('remark');
  }
  public get newRemark() {
    return this.form.get('newRemark');
  }
  //202209029新增
  public get clientStartTimePerDay() {
    return this.form.get('clientStartTimePerDay')!;
  }
  public get newClientStartTimePerDay() {
    return this.form.get('newClientStartTimePerDay')!;
  }
  public get clientEndTimePerDay() {
    return this.form.get('clientEndTimePerDay')!;
  }
  public get newClientEndTimePerDay() {
    return this.form.get('newClientEndTimePerDay')!;
  }
  public get timeZone() {
    return this.form.get('timeZone')!;
  }
  public get newTimeZone() {
    return this.form.get('newTimeZone')!;
  }

  public get clientId() {
    return this.formX.get('clientId')!;
  }
  public get apiKeyAlias() {
    return this.formX.get('apiKeyAlias')!;
  }
  public get effectiveAt() {
    return this.formX.get('effectiveAt')!;
  }
  public get expiredAt() {
    return this.formX.get('expiredAt')!;
  }
  public get groupIdList() {
    return this.formX.get('groupIdList')!;
  }
}
