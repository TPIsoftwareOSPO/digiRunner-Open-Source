...
import {
  DPB0102Req,
  DPB0102Items,
} from 'src/app/models/api/CycleScheduleService/dpb0102.interface';
...
import {
  DPB0104Req,
  DPB0104Resp,
} from 'src/app/models/api/CycleScheduleService/dpb0104.interface';
import {
  DPB0101Req,
  DPB0101Cron,
} from 'src/app/models/api/CycleScheduleService/dpb0101.interface';
import * as dayjs from 'dayjs';
import {
  DPB0103Req,
  DPB0103Resp,
} from 'src/app/models/api/CycleScheduleService/dpb0103.interface';
import {
  DPB0105Req,
  DPB0105Cron,
} from 'src/app/models/api/CycleScheduleService/dpb0105.interface';
...
export class Np0514Component extends BaseComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    let ReqBody = {
      encodeItemNo:
        this.tool.Base64Encoder(this.tool.BcryptEncoder('RJOB_STATUS')) +
        ',' +
        17,
      isDefault: 'N',
    } as DPB0047Req;
    this.list.querySubItemsByItemNo(ReqBody).subscribe(async (res) => {
      if (this.tool.checkDpSuccess(res.ResHeader)) {
        ...
        let _statusOption = [{ label: dict['all'], value: 'all' }];
    ...
    this.roleService
      .queryRTMapByUk({ txIdList: ['DPB0101', 'DPB0105'] } as DPB0115Req)
      .subscribe((res) => {
        if (this.tool.checkDpSuccess(res.ResHeader)) {
          this.canCreate = res.RespBody.dataList.find(
            (item) => item.txId === 'DPB0101',
          )
            ? res.RespBody.dataList.find((item) => item.txId === 'DPB0101')!
                .available
            : false;
          this.canUpdate = res.RespBody.dataList.find(
            (item) => item.txId === 'DPB0105',
          )
            ? res.RespBody.dataList.find((item) => item.txId === 'DPB0105')!
                .available
            : false;
        }
      });
  ...
  async init() {
    const code = [
      ...
      'week_option.monday',
      'week_option.tuesday',
      'week_option.wednesday',
      'week_option.thursday',
      'week_option.friday',
      'week_option.saturday',
    ];
    const dict = await this.tool.getDict(code);
    for (let i = 1; i < 32; i++) {
    ...
    this.scheduleCols = [
      ...
      { field: 'effPeriod', header: dict['date_duration'] },
      ...
      status:
        this.status!.value != 'all'
          ? this.tool.Base64Encoder(
              this.tool.BcryptEncoder(this.status!.value),
            ) +
            ',' +
            this.status!.value
          : '',
    ...
    switch (this.currentAction) {
      case 'create':
        let createReqBody = {
          ...
          effDateTime:
            this.effDateTime!.value != null && this.effDateTime!.value != ''
              ? dayjs(this.transformTimeSecond(this.effDateTime!.value)).format(
                  'YYYY/MM/DD HH:mm:ss',
                )
              : '',
          invDateTime:
            this.invDateTime!.value != null && this.invDateTime!.value != ''
              ? dayjs(this.transformTimeSecond(this.invDateTime!.value)).format(
                  'YYYY/MM/DD HH:mm:ss',
                )
              : '',
        } as DPB0101Req;
        ...
        this.rjobItems!.value.map((item) => {
          _rjobItems.push({
            refItemNo:
              this.tool.Base64Encoder(
                this.tool.BcryptEncoder(item.refItemNo.subitemNo),
              ) +
              ',' +
              item.refItemNo.index,
            refSubitemNo:
              item.refSubitemNo && item.refSubitemNo != ''
                ? this.tool.Base64Encoder(
                    this.tool.BcryptEncoder(item.refSubitemNo.subitemNo),
                  ) +
                  ',' +
                  item.refSubitemNo.index
                : '',
            inParams: item.inParams,
            identifData: item.identifData,
            sortBy: item.sortBy,
          });
        });
        ...
        this.scheduleService
          ...
          .subscribe(async (res) => {
            if (this.tool.checkDpSuccess(res.ResHeader)) {
              ...
              this.message.add({
                severity: 'success',
                summary: `${dict['button.create']} ${dict['schedule_job']}`,
                detail: `${dict['button.create']} ${dict['message.success']}`,
              });
      ...
      case 'update':
        ...
        this.orirjobItems!.value.map((item) => {
          _orirjobItems.push({
            apptRjobDId: item.apptRjobDId,
            lv: item.lv,
            refItemNo: item.refItemNo,
            refSubitemNo: item.refSubitemNo,
            inParams: item.inParams,
            identifData: item.identifData,
            sortBy: item.sortBy,
          });
        ...
        this.scheduleService
          ...
          .subscribe(async (res) => {
            if (this.tool.checkDpSuccess(res.ResHeader)) {
              ...
              this.message.add({
                severity: 'success',
                summary: `${dict['button.update']} ${dict['schedule_job']}`,
                detail: `${dict['button.update']} ${dict['message.success']}`,
              });
              this.queryScheduleList();
              this.changePage('query');
            }
      ...
      case 'pause':
        ...
        this.scheduleService.updateRjob(pauseReqBody).subscribe(async (res) => {
          if (this.tool.checkDpSuccess(res.ResHeader)) {
            ...
            this.message.add({
              severity: 'success',
              summary: `${dict['button.pause']} ${dict['schedule_job']}`,
              detail: `${dict['button.pause']} ${dict['message.success']}`,
            });
            this.queryScheduleList();
          }
        });
        ...
        this.scheduleService
          ...
          .subscribe(async (res) => {
            if (this.tool.checkDpSuccess(res.ResHeader)) {
              ...
              this.message.add({
                severity: 'success',
                summary: `${dict['button.active']} ${dict['schedule_job']}`,
                detail: `${dict['button.active']} ${dict['message.success']}`,
              });
              this.queryScheduleList();
            }
          });
      ...
      case 'skip':
        ...
        this.scheduleService.updateRjob(skipReqBody).subscribe(async (res) => {
          if (this.tool.checkDpSuccess(res.ResHeader)) {
            ...
            this.message.add({
              severity: 'success',
              summary: `${dict['button.skip_one']} ${dict['schedule_job']}`,
              detail: `${dict['button.skip_one']} ${dict['message.success']}`,
            });
            this.message.add({
              severity: 'success',
              summary: `${dict['next_execute_time']}：${res.RespBody.nextDateTime}`,
            });
            this.queryScheduleList();
          }
        });
        ...
        this.scheduleService
          ...
          .subscribe(async (res) => {
            if (this.tool.checkDpSuccess(res.ResHeader)) {
              ...
              this.message.add({
                severity: 'success',
                summary: `${dict['button.invalid']} ${dict['schedule_job']}`,
                detail: `${dict['button.invalid']} ${dict['message.success']}`,
              });
              this.queryScheduleList();
            }
          });
  ...
  async changePage(action: string, rowData?: DPB0102Items) {
    ...
    switch (action) {
      ...
      case 'traker':
        ...
        this.scheduleService
          .queryRjobHistory(historyReqBody)
          .subscribe((res) => {
            if (this.tool.checkDpSuccess(res.ResHeader)) {
              this.currentTitle = `${this.title} > ${dict['button.tracker']}`;
              this.pageNum = 3;
              this.scheduleHistoryData = res.RespBody;
              this.scheduleHistoryDataRowcount =
                this.scheduleHistoryData.historyList.length;
            }
          });
        break;
    }
  ...
  moreScheduleHistoryData() {
    let ReqBody = {
      apptJobId:
        this.scheduleHistoryData?.historyList[
          this.scheduleHistoryData.historyList.length - 1
        ].apptJobId,
    ...
    this.scheduleService.queryRjobHistory(ReqBody).subscribe((res) => {
      if (this.tool.checkDpSuccess(res.ResHeader)) {
        this.scheduleHistoryData!.historyList =
          this.scheduleHistoryData!.historyList.concat(
            res.RespBody.historyList,
          );
        this.scheduleHistoryDataRowcount =
          this.scheduleHistoryData!.historyList.length;
      }
  ...
  setScheduleValue(RespBody: DPB0103Resp, isDisabled: boolean) {
    ...
    this.effDateTime!.setValue(
      RespBody.effDateTime != '' ? new Date(RespBody.effDateTime!) : '',
    );
    this.invDateTime!.setValue(
      RespBody.invDateTime != '' ? new Date(RespBody.invDateTime!) : '',
    );
  ...
  public get dayRange() {
    return this.form.get('dayRange');
  }
  public get weekRange() {
    return this.form.get('weekRange');
  }
  public get hour() {
  ...
  public get effDateTime() {
    return this.form.get('effDateTime');
  }
  public get invDateTime() {
    return this.form.get('invDateTime');
  }
  public get rjobItems() {
    return this.form.get('rjobItems');
  }
  public get orirjobItems() {
    return this.form.get('orirjobItems');
  }
}
