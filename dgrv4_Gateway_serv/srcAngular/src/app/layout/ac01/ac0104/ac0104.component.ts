...
export class Ac0104Component extends BaseComponent implements OnInit {
  ...
  cols: (
    ...
    | { field: string; header: string; type: IndexStatusPipe }
    | { field: string; header: string; type: UTCDatetimeFormatPipe }
  )[] = [];
  ...
  constructor(
    ...
    this.form = this.fb.group({
      ...
      indexName: new UntypedFormControl(null, [
  ...
  ngOnInit() {
    ...
    this.serverService.queryAllIndex(req).subscribe((res) => {
      if (this.tool.checkDpSuccess(res.ResHeader)) {
        const _indexs: { label: string; value: string }[] = [];

        if (res.RespBody.indexList) {
          for (let item of res.RespBody.indexList) {
            _indexs.push({ label: item, value: item });
          }
          this.indexs = _indexs;
        }
      }
    ...
    let req = {
      ...
      isOpen:
        this.tool.Base64Encoder(
          this.tool.BcryptEncoder(JSON.stringify(action)),
        ) +
        ',' +
        this.isOpen(action),
    } as DPB0124Req;

    this.serverService.updateIndexOpenOrClose(req).subscribe(async (res) => {
  ...
  public get timePickerS() {
    return this.form.get('timePickerS');
  }
  public get dateE() {
    return this.form.get('dateE');
  }
  public get timePickerE() {
    return this.form.get('timePickerE');
  }
  public get indexName() {
    return this.form.get('indexName');
  }
}
